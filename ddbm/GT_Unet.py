...
class RelPosSelfAttention(nn.Module):
    ...
    def rel_to_abs(self, x):
        ...
        col_pad = torch.zeros((bs, heads, length, 1), dtype=x.dtype).to("cuda:1")
        x = torch.cat([x, col_pad], dim=3)
        flat_x = torch.reshape(x, [bs, heads, -1]).to("cuda:1")
        flat_pad = torch.zeros((bs, heads, length - 1), dtype=x.dtype).to("cuda:1")
...
# 定义输入参数
# input_channels = 96  # 输入图像的通道数（如RGB图像）
# output_channels = 96  # 输出通道数（如二分类任务）
# input_height = 512  # 输入图像高度
# input_width = 512  # 输入图像宽度
#
# # 初始化 GT_U_Net 模型
# model = GT_U_Net(img_ch=input_channels, output_ch=output_channels).cuda()
#
# # 随机生成一个输入张量 (batch_size=1, channels=3, height=256, width=256)
# input_tensor = torch.randn(2, input_channels, input_height, input_width).cuda()
#
# # 将输入张量传递给模型
# output_tensor = model(input_tensor)
#
# # 打印输入和输出的形状
# print(f"Input shape: {input_tensor.shape}")   # 输出 (1, 3, 256, 256)
# print(f"Output shape: {output_tensor.shape}")  # 输出形状应该是 (1, 1, 256, 256)
