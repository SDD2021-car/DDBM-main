...
class KarrasDenoiser:
    ...
    def training_bridge_losses(self, model, x_start, sigmas, model_kwargs=None, noise=None, vae=None):
        ...
        sigmas = th.rand(B).to("cuda:7") * (self.sigma_max - self.sigma_min) + self.sigma_min
        terms = {}
        sigmas = sigmas.to("cuda:7")
        dims = x_start.ndim
...
def sample_heun(
    ...
    for j, i in enumerate(indices):
        sigma_t = sigmas[i].unsqueeze(0)
        if churn_step_ratio > 0:
            # 1 step euler
            sigma_hat = (sigmas[i + 1] - sigmas[i]).unsqueeze(0) * churn_step_ratio + sigma_t
        ...
        if sigmas[i + 1] == 0:
            ...
            # Heun's method
            x_2 = x + d * dt
            sigma_heun = sigmas[i + 1].unsqueeze(0)
            denoised_2 = denoiser(x_2, sigma_heun, x_T).clamp(-1, 1)
            if pred_mode == 've':
                # d_2 =  (x_2 - denoised_2) / append_dims(sigmas[i + 1], x.ndim)
                d_2 = to_d(x_2, sigma_heun, denoised_2, x_T, sigma_max, w=guidance)
            elif pred_mode.startswith('vp'):
